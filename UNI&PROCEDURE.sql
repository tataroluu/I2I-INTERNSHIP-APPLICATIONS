...
BEGIN
    ...
    FROM SUMMER_INTERN WHERE FIRST_NAME = vs_first_name;
...
CREATE OR REPLACE PROCEDURE 
    ...
    ( 
        pis_first_name 
            IN SUMMER_INTERN.FIRST_NAME%TYPE,
                pos_university_name 
                    IN OUT SUMMER_INTERN.FIRST_NAME%TYPE 
    );
    IS
BEGIN
    SELECT UNIVERSITY_NAME 
        INTO pos_university_name
            FROM SUMMER_INTERN 
                WHERE FIRST_NAME = pis_first_name;
END; 
...
BEGIN    
    pis_first_name :='SERVET';
        find_university_name(pis_first_name, pos_university_name);
